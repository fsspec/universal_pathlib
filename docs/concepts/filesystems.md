...
fsspec uses **protocols** (also called schemes) to identify filesystem types. Each protocol corresponds to a specific filesystem implementation:
...
with fsspec.open('gzip://file.txt::s3://bucket/file.txt.gz', 'r', anon=True) as f:
    content = f.read()
```
